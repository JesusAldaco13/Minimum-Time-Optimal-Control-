# Mobil robot dynamics @ low frequency 
#          9.507
# Plant = ------- * eye(2,2)   
#        s + 1.73
# Outer loop PD with roll-off , kp = 0.869 kd = 0.396 
# Inner loop PI with roll-off,  kp = 0.096 ki= 0.519

param N; # no. of integration steps 
param ds:= 0.001; # integration step size 
set kset ordered:= 0..N; 

param x_t{kset};   # x coordinate of racetrack
param z_t{kset};   # z coordinate of racetrack
param theta_t{kset}; # orientation of the racetrack
param k_t{kset};     # curvature of the racetrack
...
##controls
c17{k in 0..N}:   v[k] <= 2.3;   
c18{k in 0..N}:   v[k] >= 0.00001;  
c21{k in 0..N}:   vref[k] <= 2;   
c22{k in 0..N}:   vref[k] >= 0.00001;    
c23{k in 0..N}:   wref[k] <= 10;  
...
c47{k in 1..N}: v[k]/r + L*(w[k])/(2*r) <= 46;  
c48{k in 1..N}: v[k]/r + L*(w[k])/(2*r) >= 0;
c49{k in 1..N}: v[k]/r - L*(w[k])/(2*r) <= 46;  
